...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    current_loss = 0.0
    total_images = 15000
    losses = [(total_images // FLAGS.train_batch_size)*10] * patience
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        # with the `merged` op.
        validation_summary, validation_accuracy, loss = sess.run(
        ...
        if (i % total_images) == 0:
          ...
          to_compare = losses[-patience]
          losses.append(current_loss)
          current_loss = 0.0
          possible_values = losses[:-patience]
          print("Values to compare to: {}".format(possible_values))
          if all([to_compare < v for v in possible_values]):
